...
class MenuData extends Generator
{
    public function generate(): Generator
    {
        ...
        $items = "\t['title' => '{$data['caption']}', 'url' => '', 'sub_menu' =>" . PHP_EOL;
        $items .= " \t\t[" . PHP_EOL;
        foreach ($data['modules'] as $module) {
            $items .= "\t\t['title' => '{$module['caption']}', 'url' => '', 'sub_menu' =>" . PHP_EOL;
            $items .= " \t\t\t[" . PHP_EOL;
            //处理 model，并创建自动控制器
            $controllers = [];
            ...
            foreach ($controllers as $controller) {
                $items .= "\t\t\t\t['title' => '{$controller['caption']}', 'url' => '{$module['name']}/{$controller['name']}/index']," . PHP_EOL;
            }
            $items .= "\t\t\t\t]" . PHP_EOL;
            $items .= "\t\t\t]," . PHP_EOL;
        }
